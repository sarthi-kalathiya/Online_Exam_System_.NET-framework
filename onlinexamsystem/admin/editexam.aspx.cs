...
public partial class editexam : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (eid == null)
            {
                Response.Redirect("/onlinexamsystem/admin/exam.aspx");
            }
            get_editcategorydrp();
            get_editsubjectdrp();
        ...
        using (SqlConnection con = new SqlConnection(s))
        {
            ...
            try
            {
                ...
                while (rd.Read())
                {
                    ...
                    drp_editsubjectexam.SelectedValue = rd["subject_fid"].ToString();
                    drp_editcategoryexam.SelectedValue = rd["category_fid"].ToString();
                    // txt_editexamdate.Text = rd["exam_date"].ToString();
                    txt_editexamdate.Text = DateTime.Parse(rd["exam_date"].ToString()).ToString("yyyy-MM-dd");
