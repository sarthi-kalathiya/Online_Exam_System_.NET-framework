...
public partial class admin_editquestion : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (qid == null)
            {
                Response.Redirect("/onlinexamsystem/admin/question.aspx");
            }
        ...
        using (SqlConnection con = new SqlConnection(s))
        {
            ...
            try
            {
                ...
                while (rd.Read())
                {
                    ...
                    drp_editexam.SelectedValue = rd["exam_fid"].ToString();
                    // rdo_editcorrectanswer.
                    ListItem selectedItem = rdo_editcorrectanswer.Items.FindByValue(rd["question_answer"].ToString());
