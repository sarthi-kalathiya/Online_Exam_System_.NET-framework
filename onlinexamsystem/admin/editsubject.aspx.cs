...
public partial class admin_editsubject : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if(sid == null)
            {
                Response.Redirect("/onlinexamsystem/admin/subject.aspx");
            }
            txt_subjectedit.Focus();
            get_categorydrp();
          //  get_categorydrpfill(Convert.ToInt32(sid));// calling for dropdown method
        ...
        using (SqlConnection con = new SqlConnection(s))
        {
            ...
            try
            {
                ...
                while (rd.Read())
                {
                    ...
                    drp_categoryedit.SelectedValue = rd["category_fid"].ToString();
                }
            }
            catch (Exception ex)
            {
...
//  public void get_categorydrpfill(int id)
//     {
//         using (SqlConnection con = new SqlConnection(s))
//         {
...
// SqlCommand cmd = new SqlCommand("select * from category where category_id in (select category_fid from subject where subject_id=@id)", con);
//  cmd.Parameters.AddWithValue("@id", id);
//  try
//  {
//      con.Open();
...
//      ListItem li = new ListItem("Select category", "-1");
//      drp_categoryedit.Items.Insert(0, li);
//      drp_categoryedit.SelectedIndex = 1;
