...
public partial class admin_question : System.Web.UI.Page
{
    ...
    public void deletequestion(int id)
    {
        ...
        using (SqlConnection con = new SqlConnection(s))
        {
            SqlCommand cmd = new SqlCommand("DELETE FROM question WHERE question_id = @questionid", con);
           cmd.Parameters.AddWithValue("@questionid", id);
            try
            {
                ...
                {
                    panel_examquestion_warning.Visible = true;
                    lbl_examquestionwarning.Text = "Something went wrong. Can't delete now"+ id;
                }
            }
            catch (Exception ex)
            {
